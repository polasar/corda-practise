...
public class DriverBasedTests {
    ...
    public void nodeTest() {
        driver(new DriverParameters().withIsDebug(true).withStartNodesInProcess(true), dsl -> {
            ...
            try {
                NodeHandle socGenHandle = handleFutures.get(0).get();
                NodeHandle natixisHandle = handleFutures.get(1).get();
                ...
                // as intended.
                assertEquals(socGenHandle.getRpc().wellKnownPartyFromX500Name(bankB.getName()).getName(), bankB.getName());
                assertEquals(natixisHandle.getRpc().wellKnownPartyFromX500Name(bankA.getName()).getName(), bankA.getName());
            } catch (Exception e) {
